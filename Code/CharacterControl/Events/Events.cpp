...
PE_IMPLEMENT_CLASS1(Event_CreateTarget, PE::Events::Event_CREATE_MESH);

void Event_CreateTarget::SetLuaFunctions(PE::Components::LuaEnvironment *pLuaEnv, lua_State *luaVM)
{
	static const struct luaL_Reg l_Event_CreateTarget[] = {
		{ "Construct", l_Construct },
		{ NULL, NULL } // sentinel
	};

	// register the functions in current lua table which is the table for Event_CreateTarget
	luaL_register(luaVM, 0, l_Event_CreateTarget);
}

int Event_CreateTarget::l_Construct(lua_State* luaVM)
{
	PE::Handle h("EVENT", sizeof(Event_CreateTarget));
	...
	// this function should only be called frm game thread, so we can use game thread thread owenrship mask
	Event_CreateTarget *pEvt = new(h) Event_CreateTarget(pContext->m_gameThreadThreadOwnershipMask);

	const char* meshName = lua_tostring(luaVM, -numArgs--);
	const char* meshPackage = lua_tostring(luaVM, -numArgs--);
	const char* movementPointName = lua_tostring(luaVM, -numArgs--);
	...
	StringOps::writeToString(movementPointName, pEvt->m_movementWayPoint, 32);
...
PE_IMPLEMENT_CLASS1(Event_Restart_Game, Event);

Event_Restart_Game::Event_Restart_Game(PE::GameContext &context)
	: Networkable(context, this)
{

}

void *Event_Restart_Game::FactoryConstruct(PE::GameContext& context, PE::MemoryArena arena)
{
	Event_Restart_Game *pEvt = new (arena) Event_Restart_Game(context);
	return pEvt;
}

int Event_Restart_Game::packCreationData(char *pDataStream)
{
	return 0;
}

int Event_Restart_Game::constructFromStream(char *pDataStream)
{
	int read = 0;
	//read += PE::Components::StreamManager::ReadMatrix4x4(&pDataStream[read], m_transform);
	return read;
...
PE_IMPLEMENT_CLASS1(Event_Tank_Aim, Event);

PE_IMPLEMENT_CLASS1(Event_Tank_Fire, Event);

PE_IMPLEMENT_CLASS1(Event_Tank_Reload, Event);

PE_IMPLEMENT_CLASS1(Event_Shell_Splash, Event);

PE_IMPLEMENT_CLASS1(Event_Client_Restart, Event);

PE_IMPLEMENT_CLASS1(Event_Create_Shell, Event);

PE_IMPLEMENT_CLASS1(Event_Get_Hit, Event);
