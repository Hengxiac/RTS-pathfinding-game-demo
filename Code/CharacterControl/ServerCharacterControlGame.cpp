...
int ServerCharacterControlGame::initGame()
{
	...
	CharacterControlContext *pGameCtx = (CharacterControlContext*) m_pContext->m_pGameSpecificContext;
	...
	//create tank controls that will be enabled if tank is activated
	{
		...
		{
			for (int i = 0; i < 6; ++i)
				((ServerGameObjectManagerAddon*)(pGameCtx->getGameObjectManagerAddon()))->createTank(
