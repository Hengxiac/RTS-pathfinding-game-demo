...
namespace CharacterControl {
	...
	namespace Components {
		...
		struct SoldierNPCBehaviorSM : public PE::Components::Component
		{
			...
			PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_UPDATE_TARGET_POS)
				virtual void do_UPDATE_TARGET_POS(PE::Events::Event *pEvt);

			PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_CHECK_SELECTED)
				virtual void do_CHECK_SELECTED(PE::Events::Event *pEvt);

			PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_PRE_RENDER_needsRC)
				void do_PRE_RENDER_needsRC(PE::Events::Event *pEvt);

			PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_UPDATE_GROUP)
				void do_UPDATE_GROUP(PE::Events::Event *pEvt);

			PE::Handle m_hMovementSM;

			bool m_havePatrolWayPoint;
			char m_curPatrolWayPoint[32];
			Queue<Vector3>m_nextPos;
