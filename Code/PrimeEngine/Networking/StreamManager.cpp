...
void StreamManager::sendNextPackets()
{
    while (true)
    {
        ...
        if (numEvents || numGhosts) //todo: other managers
        {
			...
			size += StreamManager::WriteBool(numEvents > 0, &pPacket->m_data[size]);
            ...
            // ghost manager
			size += StreamManager::WriteBool(numGhosts > 0, &pPacket->m_data[size]);
...
void StreamManager::receivePacket(Packet *pPacket)
{
	...
	PrimitiveTypes::Bool managerFlag;
	
	read += StreamManager::ReadBool(&pPacket->m_data[read], managerFlag);
	if (managerFlag)
