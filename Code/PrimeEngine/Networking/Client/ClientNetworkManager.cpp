...
void ClientNetworkManager::clientConnectToTCPServer(const char *strAddr, int port)
{
	...
	while (!created)
	{
		...
		else
		{
			...
			serAddr.sin_addr.S_un.S_addr = inet_addr(strAddr);
			
			createNetworkConnectionContext(sock, &m_netContext, (SA*) &serAddr);
...
void ClientNetworkManager::clientConnectToUDPServer(const char *strAddr, int port)
{
	...
	while (!created)
	{
		const char *err = socket_strerror(socket_create(&sock, AF_INET, SOCK_DGRAM, IPPROTO_UDP));
		//sock = serSocket;
		if (err)
		{
			assert(!"error creating socket occurred");
...
void ClientNetworkManager::createNetworkConnectionContext(t_socket sock, PE::NetworkContext *pNetContext, SA* serverAddr)
{
	NetworkManager::createNetworkConnectionContext(sock, pNetContext);

	{
		pNetContext->m_pConnectionManager = new (m_arena) ClientConnectionManager(*m_pContext, m_arena, *pNetContext, Handle(), serverAddr);
...
void ClientNetworkManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/)
{
	...
	m_netContext.getGhostManager()->debugRender(threadOwnershipMask, xoffset + dx, yoffset + dy, true);
