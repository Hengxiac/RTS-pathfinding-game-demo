...
void ServerNetworkManager::serverOpenUDPSocket()
{
	...
	while (!created)
	{
		const char *err = socket_strerror(socket_create(&m_sock, AF_INET, SOCK_DGRAM, IPPROTO_UDP));
...
void ServerNetworkManager::createNetworkConnectionContext(t_socket sock,  int clientId, PE::NetworkContext *pNetContext, SA* clientAddr)
{
	...
	{
		pNetContext->m_pConnectionManager = new (m_arena) ServerConnectionManager(*m_pContext, m_arena, *pNetContext, Handle(), clientAddr);
...
void ServerNetworkManager::do_UPDATE(Events::Event *pEvt)
{
	...
	t_socket sock;
	int err = socket_accept(&m_sock, &sock, (SA*)(&clientAddr), &nAddrlen, &timeout);
	unsigned short  int port = clientAddr.sin_port;
	if (err != IO_DONE)
	{
	...
	if (err == IO_DONE)
	{
		...
		createNetworkConnectionContext(sock, clientIndex, &netContext, (SA*) &clientAddr);
		m_connectionsMutex.unlock();

		PE::Handle h("Event_SEND_GHOST_TO_NEW_CLIENT", sizeof(Event_SEND_GHOST_TO_NEW_CLIENT));
		Events::Event_SEND_GHOST_TO_NEW_CLIENT *pEvt = new(h) Event_SEND_GHOST_TO_NEW_CLIENT();

		pEvt->m_clientId = clientIndex;
...
void ServerNetworkManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/)
{
	...
	for (unsigned int i = 0; i < m_clientConnections.m_size; ++i)
	{
		...
		netContext.getGhostManager()->debugRender(threadOwnershipMask, xoffset + dx, yoffset + dy * 2.0f + evtManagerDy * i);
	}
	m_connectionsMutex.unlock();
}
