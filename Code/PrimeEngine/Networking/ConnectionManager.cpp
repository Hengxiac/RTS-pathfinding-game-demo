...
ConnectionManager::ConnectionManager(PE::GameContext &context, PE::MemoryArena arena, PE::NetworkContext &netContext, Handle hMyself, SA* targetAddr)
...
void ConnectionManager::sendPacket(Packet *pPacket, TransmissionRecord *pTransmissionRecord)
{
	...
	while (total < count && err == IO_DONE) {
		...
		err = socket_sendto(&m_sock, &pPacket->m_data[total], step, &done, m_targetAddr, sizeof(*m_targetAddr), tm);
		total += done;
	}
...
void ConnectionManager::receivePackets()
{
    ...
    while (true)
    {
        ...
        err = socket_recvfrom(&m_sock, &m_buffer[m_bytesBuffered], PE_SOCKET_RECEIVE_BUFFER_SIZE - m_bytesBuffered, &got, NULL, NULL, tm);
