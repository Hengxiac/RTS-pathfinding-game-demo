...
int ClientGame::runGameFrame()
{
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        if (Event_UPDATE::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			Handle hpUpdateEvent("EVENT", sizeof(Event_PHYSICS_UPDATE));
			Event_PHYSICS_UPDATE *pUpdateEvent = new(hpUpdateEvent) Event_PHYSICS_UPDATE;

			pUpdateEvent->m_frameTime = m_frameTime;
			//Push event into general queue
			Events::EventQueueManager::Instance()->add(hpUpdateEvent, Events::QT_GENERAL);
        }
		else if (Event_PHYSICS_UPDATE::GetClassId() == pGeneralEvt->getClassId())
		{
        ...
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				drawEvt->m_frustumPlane[0] = pcam->m_frustumPlane[0];
				drawEvt->m_frustumPlane[1] = pcam->m_frustumPlane[1];
				drawEvt->m_frustumPlane[2] = pcam->m_frustumPlane[2];
				drawEvt->m_frustumPlane[3] = pcam->m_frustumPlane[3];
				drawEvt->m_frustumPlane[4] = pcam->m_frustumPlane[4];
				drawEvt->m_frustumPlane[5] = pcam->m_frustumPlane[5];
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				preRenderEvt.m_frustumPlane[0] = pcam->m_frustumPlane[0];
				preRenderEvt.m_frustumPlane[1] = pcam->m_frustumPlane[1];
				preRenderEvt.m_frustumPlane[2] = pcam->m_frustumPlane[2];
				preRenderEvt.m_frustumPlane[3] = pcam->m_frustumPlane[3];
				preRenderEvt.m_frustumPlane[4] = pcam->m_frustumPlane[4];
				preRenderEvt.m_frustumPlane[5] = pcam->m_frustumPlane[5];
				...
				if (DebugRenderer::Instance()->m_renderOpen)
				{
					DebugRenderer::Instance()->createRootLineMesh();// send event while the array is on the stack

					s_context.getNavMeshManager()->drawNavMesh();

				}
				// draw physics Component
				s_context.getPhysicsManager()->drawComponents();
		...
		else if (Event_GET_WORLD_POS::GetClassId() == pGeneralEvt->getClassId())
		{
			Event_GET_WORLD_POS *pRealEvent = (Event_GET_WORLD_POS *)(pGeneralEvt);

			pcam->calculateScreenMatrix();

			Matrix4x4 screenToWorldTransform = (pcam->m_projectedToScreenTransform * pcam->m_viewToProjectedTransform * pcam->m_worldToViewTransform).inverse();

			//Vector3 a{ 0.0,0.0,0.0 };
			//Vector3 e = pcam->m_viewToProjectedTransform * pcam->m_worldToViewTransform * a;
			//Vector3 screenPos{ pRealEvent->x, pRealEvent->y , 1.0f };

			//Vector3 worldPos = screenToWorldTransform * screenPos;
			Ray cameraRay = Ray(pcam->m_base.getPos(), screenToWorldTransform * Vector3{ pRealEvent->x, pRealEvent->y , 1.0f });

			Vector3 worldPos = cameraRay.getPosForY0();

			Handle htpupdateEvent("EVENT", sizeof(Event_UPDATE_TARGET_POS));
			Event_UPDATE_TARGET_POS *tpupdateEvent = new(htpupdateEvent) Event_UPDATE_TARGET_POS;

			tpupdateEvent->m_targetPos.m_x = worldPos.m_x;
			tpupdateEvent->m_targetPos.m_y = worldPos.m_y;
			tpupdateEvent->m_targetPos.m_z = worldPos.m_z;
			tpupdateEvent->deleteOld = pRealEvent->deleteOld;
			tpupdateEvent->patrolling = pRealEvent->patrolling;

			m_pContext->getGameObjectManager()->handleEvent(tpupdateEvent);

		}

		else if (Event_SELECT_UNIT::GetClassId() == pGeneralEvt->getClassId())
		{
			Event_SELECT_UNIT *pRealEvent = (Event_SELECT_UNIT *)(pGeneralEvt);
			...
			Handle hcheckSelEvent("EVENT", sizeof(Event_CHECK_SELECTED));
			Event_CHECK_SELECTED *pcheckSelEvent = new(hcheckSelEvent) Event_CHECK_SELECTED;

			pcheckSelEvent->multiSelect = pRealEvent->multiSelect;

			pcheckSelEvent->m_origin = pcam->m_base.getPos();
			pcheckSelEvent->m_target = screenToWorldTransform * Vector3{ pRealEvent->x, pRealEvent->y , 1.0f };

			m_pContext->getGameObjectManager()->handleEvent(pcheckSelEvent);
		...
		else if (Event_CAMERA_STORAGE::GetClassId() == pGeneralEvt->getClassId())
		{
			Event_CAMERA_STORAGE *pRealEvent = (Event_CAMERA_STORAGE *)(pGeneralEvt);

			if (pRealEvent->isStore)
				CameraManager::Instance()->storeCurrentCamera(pRealEvent->num);
			else
				CameraManager::Instance()->setActiveCamera(pRealEvent->num);

		}

		else if (Event_CONTROL_GROUP::GetClassId() == pGeneralEvt->getClassId())
		{
			Event_CONTROL_GROUP *pRealEvent = (Event_CONTROL_GROUP *)(pGeneralEvt);

			if (pRealEvent->isStore)
			{
				m_pContext->getGameObjectManager()->m_groupControl[pRealEvent->num - 1].clear();
			}

			Handle hupdatGEvent("EVENT", sizeof(Event_UPDATE_GROUP));
			Event_UPDATE_GROUP *pupdateGEvent = new(hupdatGEvent) Event_UPDATE_GROUP;

			pupdateGEvent->num = pRealEvent->num - 1;

			pupdateGEvent->isStore = pRealEvent->isStore;
			m_pContext->getGameObjectManager()->handleEvent(pupdateGEvent);

		}
		else if (Event_TOGGLE_DEBUG_RENDER::GetClassId() == pGeneralEvt->getClassId())
		{
			DebugRenderer::Instance()->m_renderOpen = !DebugRenderer::Instance()->m_renderOpen;
		}
