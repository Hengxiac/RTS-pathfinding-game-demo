...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	else if (Event_MOUSE_RIGHT_HELD::GetClassId() == pEvt->getClassId())
	{
		PE::Events::Event_MOUSE_RIGHT_HELD *pRealEvt = (PE::Events::Event_MOUSE_RIGHT_HELD *)(pEvt);

		Handle h("EVENT", sizeof(Event_GET_WORLD_POS));

		Event_GET_WORLD_POS *getPosEvt = new(h) Event_GET_WORLD_POS;

		getPosEvt->deleteOld = !pRealEvt->shiftHeld;
		getPosEvt->patrolling = pRealEvt->pHeld;
		getPosEvt->x = pRealEvt->x;
		getPosEvt->y = pRealEvt->y;

		m_pQueueManager->add(h, QT_GENERAL);

	}

	else if (Event_MOUSE_LEFT_HELD::GetClassId() == pEvt->getClassId())
	{
		PE::Events::Event_MOUSE_LEFT_HELD *pRealEvt = (PE::Events::Event_MOUSE_LEFT_HELD *)(pEvt);

		Handle h("EVENT", sizeof(Event_SELECT_UNIT));

		Event_SELECT_UNIT *selectUnitEvt = new(h) Event_SELECT_UNIT;

		selectUnitEvt->multiSelect = pRealEvt->ctrlHeld;

		selectUnitEvt->x = pRealEvt->x;
		selectUnitEvt->y = pRealEvt->y;
	...
	{
		Handle h("EVENT", sizeof(Event_TOGGLE_DEBUG_RENDER));

		Event_TOGGLE_DEBUG_RENDER *toggleDREvt = new(h) Event_TOGGLE_DEBUG_RENDER;

		m_pQueueManager->add(h, QT_GENERAL);

	}

	else if (Event_FUN_KEY_HELD::GetClassId() == pEvt->getClassId())
	{
		PE::Events::Event_FUN_KEY_HELD *pRealEvt = (PE::Events::Event_FUN_KEY_HELD *)(pEvt);

		Handle h("EVENT", sizeof(Event_CAMERA_STORAGE));

		Event_CAMERA_STORAGE *cameraSEvt = new(h) Event_CAMERA_STORAGE;

		cameraSEvt->num = pRealEvt->num;

		cameraSEvt->isStore = pRealEvt->ctrlHeld;

		m_pQueueManager->add(h, QT_GENERAL);

	}

	else if (Event_NUM_KEY_HELD::GetClassId() == pEvt->getClassId())
	{
		PE::Events::Event_NUM_KEY_HELD *pRealEvt = (PE::Events::Event_NUM_KEY_HELD *)(pEvt);

		Handle h("EVENT", sizeof(Event_CONTROL_GROUP));

		Event_CONTROL_GROUP *cGEvt = new(h) Event_CONTROL_GROUP;

		cGEvt->num = pRealEvt->num;

		cGEvt->isStore = pRealEvt->ctrlHeld;
