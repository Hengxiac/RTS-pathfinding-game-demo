	...
	int ClientGame::initEngine(GameContext &context, PE::MemoryArena arena, EngineInitParams &engineParams)
{
	...
	*/
...

	{
		...
		Handle hnm("NavMeshManager", sizeof(NavMeshManager));
		context.m_pNavMeshManager = new (hnm) NavMeshManager(context, arena, hnm);
		context.getNavMeshManager()->addDefaultComponents();
	}

	{
		Handle hp("PhysicsManager", sizeof(PhysicsManager));
		context.m_pPhysicsManager = new (hp) PhysicsManager(context, arena, hp);
...
int ClientGame::runGame()
{
	...
	ClientNetworkManager *pClientNetwManager = (ClientNetworkManager *)(m_pContext->getNetworkManager());
	pClientNetwManager->clientConnectToTCPServer("127.0.0.1", 1660);
