...
struct CameraSceneNode : public SceneNode
{
	...
	void calculateScreenMatrix(); //call this function before using m_projectedToScreenTransform
	// Individual events -------------------------------------------------------
	...
	float m_near, m_far;
	Matrix4x4 m_projectedToScreenTransform; // objects in actual screen space are multiplied by this to get them to normalized screen space
	Vector4 m_frustumPlane[6]; //left right bottom top near far
};
}; // namespace Components
}; // namespace PE
#endif
