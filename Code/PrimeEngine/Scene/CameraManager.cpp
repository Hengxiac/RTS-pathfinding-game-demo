...
void CameraManager::storeCurrentCamera(int i) {
	Camera* pCam = getActiveCamera();
	if (pCam)
	{
		PE::Handle storeLoc = storedCamera[i];
		Camera* pSCam = storeLoc.getObject<Camera>();
		if (!pSCam->getCamSceneNode())
		{
			pSCam->m_hCameraSceneNode = Handle("CAMERA_SCENE_NODE", sizeof(CameraSceneNode));
			CameraSceneNode* pSN = new(pSCam->m_hCameraSceneNode) CameraSceneNode(*m_pContext, m_arena, pSCam->m_hCameraSceneNode);
		}
		memcpy(pSCam->getCamSceneNode(), pCam->getCamSceneNode(), sizeof(CameraSceneNode));
		CameraSceneNode* a = pSCam->getCamSceneNode();
		CameraSceneNode* b = pCam->getCamSceneNode();
		float c = 0;
	}
}
...
void CameraManager::setActiveCamera(int i) {
	...
	if (pCam)
	{
		if (storedCamera[i].isValid())
		{
			PE::Handle storeLoc = storedCamera[i];
			Camera* pSCam = storeLoc.getObject<Camera>();
			if (pSCam->getCamSceneNode())
			{
				memcpy(pCam->getCamSceneNode(), pSCam->getCamSceneNode(), sizeof(CameraSceneNode));
			}
		}
