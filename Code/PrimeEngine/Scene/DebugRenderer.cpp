...
, m_numFreeing(0), m_renderOpen(false)
{
...
void DebugRenderer::createBoundingBoxMesh(bool hasTransform, const Matrix4x4 &transform, Vector3* boundingCoord, Vector3 color, float timeToLive, float scale /* = 1.0f*/)
{
	Vector3 linepts_1[] = { boundingCoord[0], color, boundingCoord[4], color };
	createLineMesh(hasTransform, transform, &linepts_1[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_2[] = { boundingCoord[0], color, boundingCoord[2], color };
	createLineMesh(hasTransform, transform, &linepts_2[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_3[] = { boundingCoord[0], color, boundingCoord[1], color };
	createLineMesh(hasTransform, transform, &linepts_3[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_4[] = { boundingCoord[1], color, boundingCoord[3], color };
	createLineMesh(hasTransform, transform, &linepts_4[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_5[] = { boundingCoord[1], color, boundingCoord[5], color };
	createLineMesh(hasTransform, transform, &linepts_5[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_6[] = { boundingCoord[2], color, boundingCoord[6], color };
	createLineMesh(hasTransform, transform, &linepts_6[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_7[] = { boundingCoord[2], color, boundingCoord[3], color };
	createLineMesh(hasTransform, transform, &linepts_7[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_8[] = { boundingCoord[3], color, boundingCoord[7], color };
	createLineMesh(hasTransform, transform, &linepts_8[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_9[] = { boundingCoord[4], color, boundingCoord[5], color };
	createLineMesh(hasTransform, transform, &linepts_9[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_10[] = { boundingCoord[4], color, boundingCoord[6], color };
	createLineMesh(hasTransform, transform, &linepts_10[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_11[] = { boundingCoord[5], color, boundingCoord[7], color };
	createLineMesh(hasTransform, transform, &linepts_11[0].m_x, 2, timeToLive, scale);

	Vector3 linepts_12[] = { boundingCoord[6], color, boundingCoord[7], color };
	createLineMesh(hasTransform, transform, &linepts_12[0].m_x, 2, timeToLive, scale);
