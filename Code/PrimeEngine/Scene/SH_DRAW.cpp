...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	if (pMeshCaller->m_performBoundingVolumeCulling  && !Z_ONLY)
	{
		pMeshCaller->m_numVisibleInstances = 0;
		...
		for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
		{
			...
			Mesh *pMesh = pInst->getFirstParentByTypePtr<Mesh>();
			Handle hSN = pInst->getFirstParentByType<SceneNode>();
			if (!hSN.getObject<PE::Components::SceneNode>()) {
			...
			else
			{
				...
				// x0y0z0 x1y0z0 x0y1z0 x1y1z0 x0y0z1 x1y0z1 x0y1z1 z1y1z1
				Vector3 boundingCoord[8];


				// find the bounding volume in world space
				boundingCoord[0] = base.getPos() + pMesh->m_volumnBoundingCoord[0] * base.getU() + pMesh->m_volumnBoundingCoord[1] * base.getV() + pMesh->m_volumnBoundingCoord[2] * base.getN();
				boundingCoord[1] = base.getPos() + pMesh->m_volumnBoundingCoord[3] * base.getU() + pMesh->m_volumnBoundingCoord[1] * base.getV() + pMesh->m_volumnBoundingCoord[2] * base.getN();
				boundingCoord[2] = base.getPos() + pMesh->m_volumnBoundingCoord[0] * base.getU() + pMesh->m_volumnBoundingCoord[4] * base.getV() + pMesh->m_volumnBoundingCoord[2] * base.getN();
				boundingCoord[3] = base.getPos() + pMesh->m_volumnBoundingCoord[3] * base.getU() + pMesh->m_volumnBoundingCoord[4] * base.getV() + pMesh->m_volumnBoundingCoord[2] * base.getN();
				boundingCoord[4] = base.getPos() + pMesh->m_volumnBoundingCoord[0] * base.getU() + pMesh->m_volumnBoundingCoord[1] * base.getV() + pMesh->m_volumnBoundingCoord[5] * base.getN();
				boundingCoord[5] = base.getPos() + pMesh->m_volumnBoundingCoord[3] * base.getU() + pMesh->m_volumnBoundingCoord[1] * base.getV() + pMesh->m_volumnBoundingCoord[5] * base.getN();
				boundingCoord[6] = base.getPos() + pMesh->m_volumnBoundingCoord[0] * base.getU() + pMesh->m_volumnBoundingCoord[4] * base.getV() + pMesh->m_volumnBoundingCoord[5] * base.getN();
				boundingCoord[7] = base.getPos() + pMesh->m_volumnBoundingCoord[3] * base.getU() + pMesh->m_volumnBoundingCoord[4] * base.getV() + pMesh->m_volumnBoundingCoord[5] * base.getN();

				bool out_of_frustum = false;
				Vector4 plane;
				for (int j = 0; j < 6; j++) {
					plane = pDrawEvent->m_frustumPlane[j];
					out_of_frustum = true;
					for (int i = 0; i < 8; i++) {
						out_of_frustum = out_of_frustum && (boundingCoord[i].m_x * plane.m_x +
							boundingCoord[i].m_y * plane.m_y + boundingCoord[i].m_z * plane.m_z + plane.m_w) < 0;
					}
